...
/*
	Função de Recorrência:
		opt[j] = min(opt[i-1] + costLine(i,j)), para i variando entre 1 e j.
	Onde:
		opt[j] é o custo mínimo para um texto com as j primeiras palavras de "words"
		costLine(i,j) é o custo de uma linha iniciando na palavra i e terminando em j
*/
vector<string> words;
int line_width;

void solve(vector<string> wrds, int width){
	words = wrds;
	line_width = width;
	...
	for(j = 1; j <= n; ++j){
		opt[j] = INF;
		for(i = 1; i <= j; ++i){
			int new_total = opt[i-1] + costLine(i,j);
			if(new_total < opt[j]){ 
				opt[j] = new_total;
				parent[j] = i;
			}
...
int costLine(int i, int j){
	...
	if(filled > line_width){
		...
		return (line_width - filled) * (line_width - filled);
	}
}
